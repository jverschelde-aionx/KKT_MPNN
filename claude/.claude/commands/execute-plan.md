...
1. Check if it already exists in Archon using `mcp__archon__find_tasks(...)`
2. Create a corresponding task in Archon using `mcp__archon__manage_task("create", ...)` only if the task doesnt exist yet
3. Set initial status as "todo"
4. Include detailed descriptions from the plan
5. Maintain the task order/priority from the plan
...
### 5.3 Complete Task
- Once implementation is complete, first git commit and push the changes, next move task to "review" status: `mcp__archon__manage_task("update", task_id=..., status="review")`
