...
def agent_entry() -> None:
    pretrain_run = wandb.init(project="kkt_gnn_node_pretraining")
    assert pretrain_run is not None, "wandb.init() failed"
    ...
    save_dir = train(overrides=overrides)

    baseline_dir = save_dir.replace("pretrain", "baseline")
    if not Path.exists(baseline_dir):
        baseline_run = wandb.init(project="kkt_gnn_node_finetuning")
        assert baseline_run is not None, "wandb.init() failed"
        ...
        overrides["encoder_path"] = None
        overrides["finetune_mode"] = "full"
        finetune(overrides=overrides)

    finetune_run = wandb.init(project="kkt_gnn_node_finetuning")
    assert finetune_run is not None, "wandb.init() failed"
    ...
    overrides["encoder_path"] = Path(save_dir) / "best_encoder.pt"
    overrides["finetune_mode"] = "full"
    finetune(overrides=overrides)
...
def main() -> None:
    ...
    parser.add_argument(
        "--project",
        type=str,
        default="kkt_gnn_node_pretraining_finetune",
