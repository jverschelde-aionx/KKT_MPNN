...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        valid_input_mask: torch.Tensor = None,
        mask_value=-1e6,
    ):
        ...
        if valid_input_mask is not None:
            att = att.masked_fill(
                valid_input_mask.unsqueeze(1).unsqueeze(2) == 0, mask_value
            )
...
class MaskedTransformerBlock(nn.Module):
    def __init__(
        self, n_layer, n_embd, n_ff, n_head, attn_pdrop, resid_pdrop, prenorm=True
    ):
        super().__init__()
        self.blocks = nn.ModuleList(
            [
                Block(n_embd, n_ff, n_head, attn_pdrop, resid_pdrop, prenorm)
...
class MaskedOnlyTransformerEncoder(nn.Module):
    ...
    def forward(self, x, attn_mask=None, valid_input_mask=None):
        ...
        """
        x = self.masked_transformer(
            x, attn_mask=attn_mask, valid_input_mask=valid_input_mask
        )
        return x
