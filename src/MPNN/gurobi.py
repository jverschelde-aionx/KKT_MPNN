...
def solve_grb(filepath, log_dir, settings):
    ...
    try:
        ...
        m.dispose()  # frees Gurobi memory immediately
        del m


def collect(ins_dir, q, sol_dir, log_dir, bg_dir, skip_solve, settings):
    while True:
        filename = q.get()
        if not filename:
            break
...
if __name__ == "__main__":
    ...
    for size in sizes:
        ...
        for i in range(N_WORKERS):
            p = Process(
                target=collect,
                args=(INS_DIR, q, SOL_DIR, LOG_DIR, BG_DIR, SKIP_SOLVE, SETTINGS),
            )
            p.start()
            ps.append(p)
        for p in ps:
