...
if __name__ == "__main__":
    ...
    for size in sizes:
        ...
        INS_DIR = os.path.join(dataDir, f"data/instance/train/{size}")

        if not os.path.isdir(f"./data/dataset/{size}"):
            os.mkdir(f"./data/dataset/{size}")
        if not os.path.isdir(f"./data/dataset/{size}/solution"):
            os.mkdir(f"./data/dataset/{size}/solution")
        if not os.path.isdir(f"./data/dataset/{size}/NBP"):
            os.mkdir(f"./data/dataset/{size}/NBP")
        if not os.path.isdir(f"./data/dataset/{size}/logs"):
            os.mkdir(f"./data/dataset/{size}/logs")
        if not os.path.isdir(f"./data/dataset/{size}/BG"):
            os.mkdir(f"./data/dataset/{size}/BG")

        SOL_DIR = f"./data/dataset/{size}/solution"
        LOG_DIR = f"./data/dataset/{size}/logs"
        BG_DIR = f"./data/dataset/{size}/BG"
        ...
        for i in range(N_WORKERS):
            p = Process(
                target=collect, args=(INS_DIR, q, SOL_DIR, LOG_DIR, BG_DIR, SETTINGS)
            )
            p.start()
            ps.append(p)
        for p in ps:
